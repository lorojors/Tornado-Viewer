const EF_COLORS={0:'#4caf50',1:'#cddc39',2:'#ff9800',3:'#f44336',4:'#9c27b0',5:'#ffffff'};
...
function renderMap(){
  if(!map){
    map=L.map('map',{zoomControl:true,attributionControl:false,preferCanvas:true}).setView([37,-92],4);
  ...
  allData.forEach(d=>{
    ...
    if(hasEnd){
      const line=L.polyline([[d.start_lat,d.start_lon],[d.end_lat,d.end_lon]],{color,weight,opacity:0.8,interactive:true});
      line.on('click',()=>selectTornado(d._idx));layers.push(line);
    }
    const size=5+(ef*2);
    ...
    const marker=L.marker([d.start_lat,d.start_lon],{icon,interactive:true});
    marker.bindPopup(buildPopup(d),{maxWidth:280});
    marker.on('click',()=>selectTornado(d._idx));layers.push(marker);
    mapLayers[d._idx]=L.layerGroup(layers).addTo(map);
  });
  fitMapToBounds();
}
